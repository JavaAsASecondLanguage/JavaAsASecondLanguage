...
public class IntervalsMerger {
    /**
     * Given array of intervals, merge overlapping intervals and sort by start in ascending order
     * Interval is defined as [start, end] where start < end
     *
     * <p>Examples: [[1,3][2,4][5,6]] -> [[1,4][5,6]] [[1,2][2,3]] -> [[1,3]]
     * [[1,4][2,3]] -> [[1,4]] [[5,6][1,2]] -> [[1,2][5,6]]
     *
    ...
    public int[][] merge(int[][] intervals) {
        ...
        int currentResInterval = 0;
        for (int i = 1; i < intervals.length; ++i) {
            if (intervals[i][0] <= result.get(currentResInterval)[1]) {
                if (intervals[i][1] > result.get(currentResInterval)[1]) {
                    result.get(currentResInterval)[1] = intervals[i][1];
                }
            } else {
