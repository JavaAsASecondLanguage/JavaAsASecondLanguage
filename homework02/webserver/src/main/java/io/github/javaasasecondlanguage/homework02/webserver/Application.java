...
public class Application {
    public static void initDI() {
        final int STACK_LVL_CALL = 8;
        Supplier<WebServer> webServerFactory = () -> new MyWebServer();
        Supplier<Map<String, HttpHandler>> handlerFactory = () -> Map.of("/test", new MyHttpHandler());
        Supplier<ExecutorService> executorServiceFactory = () -> Executors.newFixedThreadPool(10);
        ...
        Supplier<java.util.logging.Logger> stdLogger = () -> java.util.logging.Logger.getLogger(
                                                                    Thread.currentThread()
                                                                            .getStackTrace()[STACK_LVL_CALL]
                                                                            .getClassName());
